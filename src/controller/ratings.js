...
const createRating = ({ idUser, comment, score, idTraining }) => {
  ...
  return Ratings.create({
    ...
    Trainings.findOne({ where: { idTraining } }).then(training => {
      logger.info(
        ' [ Rating Api ] checkLowestScore to do on training %s',
        training.idTraining
      );
      checkLowestScore(training, rating.score).then(() => {
        logger.info(
          ' [ Rating Api ] checkHighestScore to do on training %s',
          training.idTraining
        );
        checkHighestScore(training, rating.score).then(() => {
          logger.info(
            ' [ Rating Api ] updateAverageScore on training %s',
            training.idTraining
          );
